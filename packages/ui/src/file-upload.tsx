...
const imageUploadVariants = cva(
  "group relative isolate flex aspect-[1200/630] w-full flex-col items-center justify-center overflow-hidden bg-muted transition-all hover:bg-muted-50",
  {
    variants: {
      variant: {
...
export function FileUpload({
  ...
  return (
    ...
    >
      {loading && (
        <div className="absolute inset-0 z-[5] flex items-center justify-center rounded-[inherit] bg-muted">
          <LoadingSpinner />
        </div>
      )}
      <div
      ...
      <div
        className={cn(
          "absolute inset-0 z-[3] flex flex-col items-center justify-center rounded-[inherit] bg-muted/75 transition-all",
          dragActive &&
            "cursor-copy border-2 border-primary bg-muted/75 opacity-100",
          imageSrc
            ? cn("opacity-0", showHoverOverlay && "group-hover:opacity-100")
            : "group-hover:bg-muted/75",
        )}
      >
        <UploadCloud
          className={cn(
